...
public class ReceiptDBController {
	...
	/**
	 * This is a method by which you can get all the necessary information related
	 * to receipts
	 * 
	 * @param clientMsg
	 * @return
	...
	public Message parseData(Message clientMsg) {
		try {
			...
			// this case is to update the receipts table with the amount of visitors that
			// already exit
			case UpdateReceiptInfoAfterExit:
					...
					if (Integer.parseInt(infoReceipt.get(2)) > 0 && currAmount >= 0) {
						...
						if (currAmount == 0) {
							...
							pstm = sqlConnection.connection.prepareStatement(
									"UPDATE  receipts SET currAmountOfVisitorsLeft=? ,visitExit=?  where receiptsID=?");
				...
				return new Message(OperationType.UpdateReceiptInfoAfterExit, ClientControllerType.ReceiptController,
						(Object) "faild to update exit");
	...
	private ResultSet checkReceipt(Object obj) throws SQLException {
		...
		try {
			pstm = sqlConnection.connection
					.prepareStatement("SELECT * from receipts where parkName=? and receiptsID=?");
