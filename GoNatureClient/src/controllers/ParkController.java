...
/**
 * This class manages the details of each park in the database include capacity,
 * update park parameters, the class use the returend parameters that recieved
	...
	public static void ParkParseData(Message reciveMsg) {
		...
		case GetOrderInfo:
			...
			if (reciveMsg.getObj() instanceof Order) {
				...
				if ((order.getStatus().equals("Approved")) && order.getDate().equals(thisDayToDB)
						&& order.getHourTime() + 4 >= hours && order.getHourTime() <= hours) {
